...
describe('RiTa.RiScript', () => {
  ...
  describe('Inline', () => {
    ...
    it('Should evaluate inline assigns', () => {
      ...
      expect(RiTa.evaluate('$a=a\n($a).toUpperCase()', ctx = {}, 0)).eq('A');

      expect(RiTa.evaluate('[$stored=(a | a)] dog is a mammal', ctx, 0)).eq(exp.toLowerCase());
  ...
  describe('Symbol', () => {
    ...
    it('Should resolve symbols in context', () => {
      ...
      expect(RiTa.evaluate('$100 is a lot of $dog.', { dog: 'terrier' }, 0)).eq('$100 is a lot of terrier.');
      expect(RiTa.evaluate('the $dog cost $100', { dog: 'terrier' }, 0)).eq('the terrier cost $100');
      expect(RiTa.evaluate('the $dog cost $100!', { dog: 'terrier' }, 0)).eq('the terrier cost $100!');
      expect(RiTa.evaluate('the $dog cost ***lots***', { dog: 'terrier' }, 0)).eq('the terrier cost ***lots***');
      expect(RiTa.evaluate('the $dog^1 was a footnote.', { dog: 'terrier' }, 0)).eq('the terrier^1 was a footnote.');
    });
  ...
  describe('Transform', () => {
    ...
    it('Should handle choice transforms', () => {
      ...
      expect(RiTa.evaluate('((a)).toUpperCase()', 0, 0)).eq('A');
