const Grammar = require('../src/newgrammar');

const ADD_WEIGHTS = 0;

describe('RiTa.NewGrammar', () => {
    ...
    it("should correctly call multeval", () => {
        let ctx, exp;

        ctx = { a: 'a', b: 'b' };
        expr = '(a|a)';
        expect(new Grammar().multeval(expr, ctx)).eq('a');

        ctx = { a: '$b', b: '(c | c)' };
        expr = '$a';
        expect(new Grammar().multeval(expr, ctx)).eq('c');
        ...
        expect(new Grammar().multeval('$s', ctx)).eq('c');
    });
