...
describe("RiTa.Analyzer", () => {
  ...
  it("Should call Util.numberToWords", () => {
    ...
    for (var i = 0; i < tests.length; i++) {
      expect(Util.numberToWords(tests[i])).equals(expected[i]);
    }
  });
  ...
  it("Should call analyzeWord", () => {
    ...
    let data = RiTa.analyzer.analyzeWord("abandon"); // SYNC:
  ...
  it("Should call analyze", () => {
    ...
    expect(feats.syllables).eq(
      hasLex ? "sh-eh-v/r-ow/l-ey" : "ch-eh-v/r-ow/l-ah-t"
    );
  ...
  it("Should call stresses", () => {
    ...
    eq(RiTa.stresses("The emperor had no clothes on."), "0 1/0/0 1 1 1 1 .");
    eq(
      RiTa.stresses("The emperor had no clothes on. The King is fat."),
  ...
  it("Should call phones", () => {
    ...
    answer = hasLex
      ...
      : "dh-ah b-oy jh-ah-m-p-t ow-v-er dh-ah w-ay-l-d d-aa-g .";
    eq(result, answer);

    result = RiTa.phones("The boy ran to the store.");
    answer = hasLex
      ? "dh-ah b-oy r-ae-n t-uw dh-ah s-t-ao-r ."
      : "dh-ah b-oy r-ah-n t-ow dh-ah s-t-ao-r .";
    eq(result, answer);
    ...
    answer = hasLex
      ...
      : "dh-ah d-aa-g r-ah-n f-ae-s-t-er th-ae-n dh-ah ah-dh-er d-aa-g . b-ah-t dh-ah ah-dh-er d-aa-g w-ah-z p-r-eh-t-iy-er .";
    eq(result, answer);

    eq(RiTa.phones("quiche"), hasLex ? "k-iy-sh" : "k-w-ih-sh");
    eq(RiTa.phones("said"), hasLex ? "s-eh-d" : "s-ey-d");
    eq(
      RiTa.phones("chevrolet"),
      hasLex ? "sh-eh-v-r-ow-l-ey" : "ch-eh-v-r-ow-l-ah-t"
    );
    eq(RiTa.phones("women"), hasLex ? "w-ih-m-eh-n" : "w-ow-m-eh-n");
    eq(
      RiTa.phones("genuine"),
      hasLex ? "jh-eh-n-y-uw-w-ah-n" : "jh-eh-n-y-ah-ay-n"
    );
  ...
  it("Should call syllables", () => {
    ...
    expect(RiTa.syllables("deforestations")).eq(
      hasLex
        ? "d-ih/f-ao/r-ih/s-t-ey/sh-ah-n-z"
        : "d-ah/f-ao-r/s-t-ey/sh-ah-n-z"
    );
    expect(RiTa.syllables("chevrolet")).eq(
      hasLex ? "sh-eh-v/r-ow/l-ey" : "ch-eh-v/r-ow/l-ah-t"
    );
    expect(RiTa.syllables("women")).eq(hasLex ? "w-ih/m-eh-n" : "w-ow/m-eh-n");
    expect(RiTa.syllables("genuine")).eq(
      hasLex ? "jh-eh-n/y-uw/w-ah-n" : "jh-eh-n/y-ah/ay-n"
    );

    input = "The emperor had no clothes on.";
    expected = hasLex
      ? "dh-ah eh-m/p-er/er hh-ae-d n-ow k-l-ow-dh-z aa-n ."
      : "dh-ah eh-m/p-er/er hh-ae-d n-ow k-l-ow-dh-z ah-n .";
    expect(RiTa.syllables(input)).eq(expected);
  ...
  it("Should handle number (singular/plural)", () => {
    ...
    for (let i = 0; i < testPairs.length; i += 2) {
      ...
      res3 = RiTa.inflector.isPlural(testPairs[i], {
        dbug: dbug,
        fatal: false,
      });
