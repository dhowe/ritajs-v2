...
class Conjugator {
  ...
  unconjugate(word, opts = {}) { // NAPI
    ...
    if (IRREG_VERBS_LEX_VB.hasOwnProperty(word)) {
      dbug && console.log(word + " in exceptions list 1 (in lex)");
      return IRREG_VERBS_LEX_VB[word];
    }
    else if (Object.values(IRREG_VERBS_LEX_VB).includes(word)) {
  ...
  _reset() {
    this.IRREG_VERBS_LEX_VB = IRREG_VERBS_LEX_VB;     // for tagger
