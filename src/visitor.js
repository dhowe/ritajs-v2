...
class Sequence {
  ...
  next() {
    return this.options[this.index % this.options.length];
  }
}
...
class Visitor extends RiScriptVisitor {
  ...
  visitChoice(ctx) {
    ...
    if (txs.length) {
      //console.log("TXS", txs);
      //if (txs.map(t => t.getText()).includes('seq()')) {
      let pruned = [], hit = false
      console.log("Children: ", ctx.children.length);
      for (let i = 0; i < ctx.children.length; i++) {
        //console.log("  " + i, ctx.children[i].getText());
        if (ctx.children[i].getText() !== ".seq()") pruned.push(ctx.children[i]);
        else {
          console.log("HIT*******");
          hit = true;
        }
    ...
    if (hit) {
      ctx.children = pruned;
      console.log('Remaining', ctx.children.length);
      let seq = (this.sequence || new Sequence(options));
      token = seq.next();
    }
    else {
      // then pick a random one
      token = randomElement(options) || emptyExpr();
    }*/
