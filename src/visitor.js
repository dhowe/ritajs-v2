...
class Visitor extends RiScriptVisitor {
  ...
  constructor(parent, ignoreTransforms/* tmp */) {

    super();
    this.sequences = {};
    this.parent = parent;
    this.ignoreTransforms = ignoreTransforms; // tmp
    //if (ignoreTransforms) console.log("IGNORING TRANSFORMS");
  }
  ...
  visitChoice(ctx) {
    ...
    if (!this.ignoreTransforms) token.transforms = this.inheritTransforms(token, ctx);
  ...
  visitSymbol(ctx) {
    ...
    if (!ident) {
      if (this.ignoreTransforms) return '';
    ...
    if (this.ignoreTransforms) return this.parent.normalize(text);
  ...
  handleTransforms(obj, transforms) {
    let term = obj;
    if (this.NO_TRANSFORMS) return term;
  ...
  flatten(toks) {
    ...
    return toks.reduce((acc, t) => acc += '[' +
      this.getRuleName(t) + ':' + t.getText() + ']', '');
  }

  flattenChoice(toks) {
    if (!Array.isArray(toks)) toks = [toks];
    return toks.reduce((acc, t) => acc += '[' +
      this.getRuleName(t) + ':' + t.getText() + ']', 'choice: ');
  }
